...
1. Remove the `cost_nonneg` hypothesis from
...
2. IEEE 754 subnormal-range encoder and round-trip. Normal-range
...
3. Formal complexity bound on `bitmask_dp`. The `O(n * 2^m * m)`
...
4. Worked instances are small or partly vacuous. `c40_D`, `c30_D`,
...
5. `matching_injective` name collision. In `DETRMatchingDerived`
...
6. Theorem numbering inside section comments restarts per
