...
1. Concrete PL instantiation for `L_separated_sq`. The
...
2. `ConstructiveTraining` and `IterativeTraining` as structural
...
3. Bridge precondition discharge outside DETR. The
...
4. Remove the `cost_nonneg` hypothesis from
...
5. IEEE 754 subnormal-range encoder and round-trip. Normal-range
...
6. Formal complexity bound on `bitmask_dp`. The `O(n * 2^m * m)`
...
7. Worked instances are small or partly vacuous. `c40_D`, `c30_D`,
...
8. `matching_injective` name collision. In `DETRMatchingDerived`
...
9. Theorem numbering inside section comments restarts per
